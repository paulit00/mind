    ...
    def choose_card_to_play(self):
        ...
        while True:
            try:
                ...
                value = int(input(f"{self.name}, pasirink korta (arba irasyk 0 praleisti ejimui, arba -1 panaudoti shurikena): "))
                if value == 0:
                    return None
                if value == -1:
                    return "shuriken"
                selected_card = next((card for card in self.hand if card.value == value), None)
                if selected_card:
                    if selected_card.value == min([card.value for card in self.hand]):
                        print(f"{self.name} deda {selected_card.value} korta.")
                        return selected_card
                    else:
                        print("Neteisinga korta. Pasirink maziausia korta is visu savo kortu.")
                else:
                    print("Neteisinga korta. Pasirink maziausia korta is visu savo kortu.")
            except ValueError:
    ...
    def play(self):
        while any(self.players) & (self.num_lives > 0):  # Continue until all players have no cards left
            ...
            while any(player.hand for player in self.players):
                if self.num_lives > 0: # Continue until all players have played their cards  
                    for player in self.players:
                        if player.hand:
                            card_to_play = player.choose_card_to_play()
                            if (card_to_play is not None) & (card_to_play != "shuriken"):
                                self.current_cards.append(card_to_play)
                                if sorted([card.value for card in all_dealt_cards])[:len(self.current_cards)] == [x.value for x in self.current_cards]:
                                    print("Padeta maziausia korta")
                                    player.play_card(card_to_play)
                                    if len([card.value for card in all_dealt_cards]) == len([x.value for x in self.current_cards]):
                                    ...
                                    for player in self.players:
                                        ...
                                        self.current_cards.sort(key=lambda card: card.value)
                                    self.num_lives = self.num_lives - 1
                                ...
                                continue
                        print(f"Kortos ant stalo: {[card.value for card in self.current_cards]}")
                else:
                    return
